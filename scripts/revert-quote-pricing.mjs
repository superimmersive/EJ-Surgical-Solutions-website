/**
 * Revert products to quote-only pricing (no estimated prices).
 * Run: node scripts/revert-quote-pricing.mjs
 */
...
const products = JSON.parse(productsMatch[1]).map((product) => {
  ...
  return { ...rest, priceOnRequest: true };
});
...
console.log(`Reverted ${products.length} products to contact-for-price`);
